...
def build_improved_extended_rdkit_model():
    """
    构建改进的基于扩展RDKit特征的集成学习模型
    """
    # 定义多个基础模型，使用优化的参数
    rf = RandomForestRegressor(n_estimators=150, max_depth=8, min_samples_split=5, 
                              min_samples_leaf=2, random_state=42, n_jobs=-1)
    gb = GradientBoostingRegressor(n_estimators=150, max_depth=6, learning_rate=0.05, 
...
def predict_candidates(model, le_target, selected_features_indices, scaler, selector):
    ...
    try:
        ...
        # 保存预测结果
        pred_df.to_csv('output/improved_candidate_extended_rdkit_predictions.csv', index=False)
        print("\nPredictions saved to output/improved_candidate_extended_rdkit_predictions.csv")
...
def main():
    ...
    X, y, le_target = load_and_prepare_extended_data()
    ...
    print("Building improved extended RDKit-based ensemble model...")
    model = build_improved_extended_rdkit_model()
    ...
    print("Improved extended model training and evaluation completed!")
