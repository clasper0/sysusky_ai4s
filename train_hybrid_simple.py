"""
训练简化版混合模型预测多靶点pIC50值
=========================================

该脚本训练一个简化版混合模型，结合图神经网络和分子描述符特征，
...
from hybrid_model_simple import HybridMolecularPredictor, load_descriptor_data
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if self.descriptor_features is not None:
                ...
                # 注意：这里需要根据实际的批次大小来切片
                actual_batch_size = batch.ptr.shape[0] - 1  # 通过ptr计算实际批次大小
                descriptor_batch = torch.FloatTensor(self.descriptor_features[start_idx:start_idx + actual_batch_size]).to(self.device)
            else:
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(data_loader):
                ...
                if descriptor_features is not None:
                    ...
                    # 注意：这里需要根据实际的批次大小来切片
                    actual_batch_size = batch.ptr.shape[0] - 1  # 通过ptr计算实际批次大小
                    end_idx = min(start_idx + actual_batch_size, len(descriptor_features))
    ...
    def train(self, num_epochs: int = 100, patience: int = 20):
        ...
        for epoch in range(num_epochs):
            ...
            if val_metrics['loss'] < best_val_loss:
                ...
                patience_counter = 0
                # 保存最佳模型
                torch.save(self.model.state_dict(), 'best_hybrid_model_simple.pth')
            else:
                patience_counter += 1
        ...
        # 加载最佳模型
        self.model.load_state_dict(torch.load('best_hybrid_model_simple.pth'))
        print("训练完成，已加载最佳模型")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="训练简化版混合模型预测多靶点pIC50值")
