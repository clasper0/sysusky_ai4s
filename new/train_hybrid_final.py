"""
训练最终版混合模型预测多靶点pIC50值
=========================================

该脚本训练一个最终版混合模型，结合图神经网络和分子描述符特征，
...
from hybrid_model_final import HybridMolecularPredictor, load_descriptor_data
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if self.descriptor_features is not None:
                ...
                end_idx = min(start_idx + actual_batch_size, len(self.descriptor_features))
    ...
    def train(self, num_epochs: int = 100, patience: int = 20):
        ...
        for epoch in range(num_epochs):
            ...
            if val_metrics['loss'] < best_val_loss:
                ...
                patience_counter = 0
                # 保存最佳模型
                torch.save(self.model.state_dict(), 'best_hybrid_model_final.pth')
            else:
                patience_counter += 1
        ...
        # 加载最佳模型
        self.model.load_state_dict(torch.load('best_hybrid_model_final.pth'))
        print("训练完成，已加载最佳模型")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="训练最终版混合模型预测多靶点pIC50值")
