...
def generate_example_data(num_molecules: int = 1000, output_file: str = "data/candidate_hybrid.csv"):
    ...
    for i in range(num_molecules):
        ...
        target2_pic50 = base_activity + descriptors['HBD'] * 0.3 + np.random.normal(0, 0.5)  # 氢键供体相关
        target3_pic50 = base_activity + descriptors['HBA'] * 0.25 + np.random.normal(0, 0.5)  # 氢键受体相关
        target4_pic50 = base_activity - descriptors['MolWt'] * 0.01 + np.random.normal(0, 0.5)  # 分子量相关
        target5_pic50 = base_activity + (descriptors['HBA'] + descriptors['HBD']) * 0.2 + np.random.normal(0, 0.5)  # 氢键相关
    ...
    # 重新排序列，确保符合要求的顺序
    column_order = ['SMILES', 'target1_pIC50', 'target2_pIC50', 'target3_pIC50', 
                    'target4_pIC50', 'target5_pIC50', 'MolWt', 'LogP', 'HBA', 'HBD']
    df = df[column_order]
