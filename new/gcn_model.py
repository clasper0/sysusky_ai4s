...
class MultiTaskMolecularGCN(MolecularGCN):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(MultiTaskMolecularGCN, self).__init__(
            ...
            output_dim=1,  # Use dummy value, we'll override the prediction head anyway
            **kwargs
        )
    ...
    def forward(self, data: Data) -> Dict[str, torch.Tensor]:
        ...
        # Get shared embedding - use all layers of prediction_head to get proper embedding
        shared_embedding = self.prediction_head(combined_pool)

        # For multi-task, we need to reshape if necessary
        if shared_embedding.size(-1) != hidden_dims[-1] // 2:
            # If the output dimension doesn't match expected shared embedding size,
            # project it properly
            if not hasattr(self, 'embedding_projector'):
                self.embedding_projector = nn.Linear(self.output_dim, hidden_dims[-1] // 2).to(shared_embedding.device)
            shared_embedding = self.embedding_projector(shared_embedding)
