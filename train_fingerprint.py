...
该脚本训练一个基于分子指纹的模型，用于预测分子对多个靶点的pIC50值。
针对小样本数据进行了优化，使用随机森林或XGBoost等稳定算法。
"""
...
def main():
    parser = argparse.ArgumentParser(description='训练基于指纹的多任务pIC50预测模型')
    ...
    parser.add_argument('--model-type', type=str, default='rf', choices=['rf', 'xgb'], 
                       help='模型类型: rf (随机森林) 或 xgb (XGBoost)')
    ...
    parser.add_argument('--output-dir', type=str, default='fingerprint_experiments', 
                       help='输出目录')
    ...
    print(f"开始训练指纹模型，实验名称: {args.experiment_name}")
    ...
    # 保存模型
    model_path = os.path.join(experiment_dir, 'fingerprint_model.pkl')
