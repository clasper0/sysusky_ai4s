...
class MolecularDataset(Dataset):
    ...
    def __init__(
        ...
        if target_col is None:
            # 如果未指定目标列，查找所有pIC50相关的列
            target_cols = [col for col in self.df.columns if 'pic50' in col.lower() or 'pIC50' in col]
            if not target_cols:
                # 如果没有找到pIC50列，使用前5个数值列作为目标
...
def load_example_data(data_dir: str = "data") -> Tuple[DataLoader, DataLoader, DataLoader]:
    ...
    # 定义基础分子性质列
    base_features = ['MolWt', 'LogP', 'HBA', 'HBD']
    
    # 检查是否存在这些列
    missing_features = [col for col in base_features if col not in df.columns]
    if missing_features:
        raise ValueError(f"缺少以下基础分子性质列: {missing_features}")
    
    # 选择需要的列
    target_cols = pic50_cols
    feature_cols = ['SMILES'] + target_cols + base_features
    ...
    # 设置默认的目标列
    if target_col is None:
        target_col = pic50_cols  # 使用所有pIC50列作为目标
