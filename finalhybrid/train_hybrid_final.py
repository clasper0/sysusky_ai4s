...
该脚本使用data/candidate_hybrid.csv作为数据源，
训练一个结合图神经网络和分子描述符的混合模型来预测pIC50值。
"""
...
def main():
    ...
    try:
        model = HybridMolecularPredictor(
            ...
            gcn_hidden_dims=[64, 128],  # 减少隐藏层维度
            descriptor_dim=0,       # 暂时不使用分子描述符特征，避免维度问题
            fusion_hidden_dims=[256, 128],  # 融合层维度
            num_targets=5           # 5个靶点
        )
    ...
    # 保存最终模型
    try:
        final_model_path = os.path.join(trainer.experiment_path, "final_hybrid_model.pth")
