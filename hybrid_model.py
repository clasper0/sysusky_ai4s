"""
混合模型：结合图神经网络和分子描述符进行多任务pIC50预测
=========================================================
...
class HybridMolecularPredictor(nn.Module):
    ...
    架构:
    - 图神经网络分支 (GCN) 提取分子结构特征
    - 分子描述符分支处理传统分子属性
    - 融合层将两种特征拼接并预测多靶点pIC50
    """
    ...
    def __init__(
        ...
        self.gcn_branch = MolecularGCN(
            ...
            output_dim=gcn_hidden_dims[-1] // 2,  # GCN输出维度
            dropout_rate=gcn_dropout_rate
        )
        
        # 计算融合层输入维度
        fusion_input_dim = (gcn_hidden_dims[-1] // 2) + descriptor_dim
    ...
    def forward(self, graph_data, descriptor_features=None):
        ...
        if descriptor_features is not None and self.descriptor_dim > 0:
            ...
            if descriptor_features.size(0) != graph_features.size(0):
                raise ValueError(f"描述符特征批次大小({descriptor_features.size(0)})与图特征批次大小({graph_features.size(0)})不匹配")
            # 拼接图特征和描述符特征
