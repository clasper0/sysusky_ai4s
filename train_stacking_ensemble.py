...
该脚本训练一个堆叠集成学习模型，结合分子指纹、描述符和其他特征，
用于预测分子对多个靶点的pIC50值。
"""
...
from stacking_ensemble_model import StackingEnsembleModel
from sklearn.metrics import mean_squared_error, r2_score


def main():
    parser = argparse.ArgumentParser(description='训练堆叠集成学习模型')
    ...
    parser.add_argument('--output-dir', type=str, default='stacking_ensemble_experiments', 
                       help='输出目录')
    ...
    print(f"开始训练堆叠集成模型，实验名称: {args.experiment_name}")
    ...
    # 保存模型
    model_path = os.path.join(experiment_dir, 'stacking_ensemble_model.pkl')
