...
该脚本训练一个优化的混合集成学习模型，专门针对提高测试集R2分数进行优化，
结合分子指纹、描述符和其他特征，用于预测分子对多个靶点的pIC50值。
"""
...
from optimized_ensemble_model import OptimizedEnsembleModel
from sklearn.metrics import mean_squared_error, r2_score


def main():
    parser = argparse.ArgumentParser(description='训练优化的混合集成学习模型')
    ...
    parser.add_argument('--output-dir', type=str, default='optimized_ensemble_experiments', 
                       help='输出目录')
    ...
    print(f"开始训练优化的混合集成模型，实验名称: {args.experiment_name}")
    ...
    # 保存模型
    model_path = os.path.join(experiment_dir, 'optimized_ensemble_model.pkl')
